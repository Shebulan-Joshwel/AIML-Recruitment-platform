...
from apps.authentication.permissions import IsRecruiterOrAdmin, IsCandidateOrAdmin, IsAdmin
...
class RecalculateRankingView(APIView):
    """
    Re-run ranking with custom weights for similarity and coverage.
    POST /api/ranking/recalculate/<job_id>/
    """
    permission_classes = [IsRecruiterOrAdmin, HasActiveSubscription]
    ...
    def post(self, request, pk):
        ...
        sim_weight = float(request.data.get("similarity_weight", 1.0))
        cov_weight = float(request.data.get("coverage_weight", 0.15))

        # Support 0-100 range by normalizing to 0-1 if any value > 1
        if sim_weight > 1.0 or cov_weight > 1.0:
            sim_weight /= 100.0
            cov_weight /= 100.0
        ...
        if not applications:
            return Response({"detail": "No applications to rank."}, status=status.HTTP_400_BAD_REQUEST)

        ranked_apps = rank_applications_for_job(
            job,
            applications,
...
class ClearRankingView(APIView):
    """
    Reset ranking data for a job.
    DELETE /api/ranking/clear/<job_id>/
    """
    permission_classes = [IsRecruiterOrAdmin, HasActiveSubscription]
    ...
    def delete(self, request, pk):
        ...
        JobApplication.objects.filter(job=job).update(
            ai_score=None,
            ai_rank=None,
            predicted_label=""
        )
        return Response({"detail": "Ranking data cleared."}, status=status.HTTP_200_OK)


class AdminJobManagementView(APIView):
    """Admin-only view to list and manage all jobs."""
    permission_classes = [IsAdmin]

    def get(self, request):
        qs = Job.objects.all().select_related("recruiter")
        serializer = JobSerializer(qs, many=True)
        return Response(serializer.data)
