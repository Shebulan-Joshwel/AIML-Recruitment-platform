...
export default function AdminDashboard() {
  ...
  const [jobForm, setJobForm] = useState({ title: '', description: '', is_active: true });
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('ch_admin_tab') === 'users') loadUsers();
    if (localStorage.getItem('ch_admin_tab') === 'jobs') loadAdminJobs();
  }, []);
  ...
  async function loadAdminJobs() {
    try {
      setJobsLoading(true);
      setAdminJobs(await adminListJobs());
    } catch (e) {
  ...
  async function handleToggleJobActive(jobId, currentStatus) {
    try {
      const updated = await adminUpdateJob(jobId, { is_active: !currentStatus });
      setAdminJobs(prev => prev.map(j => j.id === updated.id ? updated : j));
    } catch (e) {
      alert(e.message || 'Failed to toggle job status');
    }
  }
  ...
  function changeTab(t) {
    ...
    if (t === 'jobs' && adminJobs.length === 0) loadAdminJobs();
  }
  ...
  return (
    <div className="ci-layout">
      ...
      <div className="ch-admin-tabs">
        ...
        <button
          className={`ch-admin-tab${tab === 'jobs' ? ' ch-admin-tab-active' : ''}`}
      ...
      <main className="ch-admin-main">
        ...
        {tab === 'users' && (
          usersLoading ? (
            <p className="ch-admin-empty-hint">Loading users...</p>
          ) : (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
                <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#111827' }}>Registered Users</h2>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button className="ch-admin-new-btn" onClick={() => loadUsers()}>Refresh</button>
                </div>
              </div>
              <div className="ri-table-wrapper" style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '0.75rem', overflow: 'hidden' }}>
                <table className="ri-table">
                  ...
                  <tbody>
                    {users.map((u) => (
                      <tr key={u.user_id}>
                        ...
                        <td>{u.email}</td>
                        <td>
                           <span className="ch-status-badge" style={{ 
                             background: u.role === 'ADMIN' ? '#fee2e2' : u.role === 'RECRUITER' ? '#dcfce7' : '#d1fae5',
                             color: u.role === 'ADMIN' ? '#991b1b' : u.role === 'RECRUITER' ? '#166534' : '#065f46',
                             borderColor: u.role === 'ADMIN' ? '#f87171' : u.role === 'RECRUITER' ? '#22c55e' : '#34d399'
                           }}>
                             {u.role}
                           </span>
                        </td>
                        ...
                        <td>
                          <div className="ch-admin-card-actions" style={{ justifyContent: 'flex-start' }}>
                            <button className="ch-admin-edit-btn" onClick={() => { 
                              setEditingUser(u); 
                            ...
                            <button className="ch-admin-delete-btn" onClick={async () => {
                              ...
                              if (confirm(`Delete user ${u.name}? This action is permanent.`)) {
                                try {
        ...
        {tab === 'jobs' && (
          jobsLoading ? (
            <p className="ch-admin-empty-hint">Loading jobs…</p>
          ) : adminJobs.length === 0 ? (
            <div className="ch-admin-empty-state">
              <span className="ch-admin-empty-icon">💼</span>
                  ...
                  {adminJobs.map((j) => (
                    <tr key={j.id}>
                      <td>
                        ...
                        <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>ID: {j.id}</div>
                      </td>
                      <td>
                        <div style={{ fontWeight: 500 }}>{j.recruiter_name}</div>
                        <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>{j.recruiter_email}</div>
                      </td>
                      <td>{j.min_experience_years || 0}y</td>
                      <td>
                        <span className={`ch-status-badge ${j.is_active ? 'ch-status-active' : 'ch-status-inactive'}`} style={{ 
                          background: j.is_active ? '#dcfce7' : '#f3f4f6',
                          color: j.is_active ? '#166534' : '#4b5563',
                          borderColor: j.is_active ? '#22c55e' : '#d1d5db'
                        }}>
                      ...
                      <td>
                        <div className="ch-admin-card-actions" style={{ justifyContent: 'flex-start' }}>
                          <button className="ch-admin-edit-btn" onClick={() => {
                            setEditingJob(j);
                            setJobForm({ title: j.title, description: j.description, is_active: j.is_active });
                            setShowJobModal(true);
                          }}>Edit</button>
                          <button 
                            className={j.is_active ? "ch-admin-delete-btn" : "ch-admin-edit-btn"}
                            style={{ 
                              borderColor: j.is_active ? '#f87171' : '#3b82f6', 
                              color: j.is_active ? '#b91c1c' : '#2563eb',
                          ...
                          <button className="ch-admin-delete-btn" onClick={async () => {
                            if (confirm(`Delete job "${j.title}"?`)) {
                              try {
                                ...
                                setAdminJobs(prev => prev.filter(item => item.id !== j.id));
                              } catch (e) { alert(e.message); }
                            }
                          }}>Delete</button>
                        </div>
      ...
      {showUserModal && (
        <div className="ch-modal-overlay" onClick={() => setShowUserModal(false)}>
          <div className="ch-modal" onClick={(e) => e.stopPropagation()}>
            <div className="ch-modal-header">
              <h2 className="ch-modal-title">Edit User</h2>
              <p className="ch-modal-subtitle">Update user profile and permissions</p>
            </div>
            <form onSubmit={async (e) => {
              e.preventDefault();
              try {
                const updated = await adminUpdateUser(editingUser.user_id, userForm);
              ...
              <div className="ch-form-grid">
                <div className="ch-form-col-full">
                  <label className="ch-form-label">Full Name</label>
                  <input className="ch-form-input" required value={userForm.name} onChange={e => setUserForm({...userForm, name: e.target.value})} />
                </div>
                <div className="ch-form-col-full">
                  <label className="ch-form-label">Email Address</label>
                  <input className="ch-form-input" type="email" required value={userForm.email} onChange={e => setUserForm({...userForm, email: e.target.value})} />
                </div>
                <div className="ch-form-col-half">
                  <label className="ch-form-label">Role</label>
                  <select className="ch-form-input" value={userForm.role} onChange={e => setUserForm({...userForm, role: e.target.value})}>
              ...
              <div className="ch-modal-footer">
                <button type="button" className="ch-btn-cancel" onClick={() => setShowUserModal(false)}>Cancel</button>
                <button type="submit" className="ch-btn-save">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showJobModal && (
        <div className="ch-modal-overlay" onClick={() => setShowJobModal(false)}>
          <div className="ch-modal" onClick={(e) => e.stopPropagation()}>
            <div className="ch-modal-header">
              <h2 className="ch-modal-title">Edit Job Post</h2>
              <p className="ch-modal-subtitle">Update job details and visibility</p>
            </div>
            <form onSubmit={async (e) => {
              e.preventDefault();
              try {
                const updated = await adminUpdateJob(editingJob.id, jobForm);
                setAdminJobs(prev => prev.map(j => j.id === updated.id ? updated : j));
              ...
              <div className="ch-form-grid">
                <div className="ch-form-col-full">
                  <label className="ch-form-label">Job Title</label>
                  <input className="ch-form-input" required value={jobForm.title} onChange={e => setJobForm({...jobForm, title: e.target.value})} />
                </div>
                <div className="ch-form-col-full">
                  <label className="ch-form-label">Description</label>
                  <textarea className="ch-form-textarea" rows={6} required value={jobForm.description} onChange={e => setJobForm({...jobForm, description: e.target.value})} />
                </div>
                <div className="ch-form-col-half">
                  <label className="ch-form-label" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <input type="checkbox" checked={jobForm.is_active} onChange={e => setJobForm({...jobForm, is_active: e.target.checked})} /> Active (visible to candidates)
                  </label>
                </div>
              </div>
              <div className="ch-modal-footer">
                <button type="button" className="ch-btn-cancel" onClick={() => setShowJobModal(false)}>Cancel</button>
                <button type="submit" className="ch-btn-save">Save Changes</button>
              </div>
            </form>
          </div>
